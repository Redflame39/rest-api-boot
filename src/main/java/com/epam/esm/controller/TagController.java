...
public class TagController {
    ...
    public CollectionModel<TagDto> read(@RequestParam(name = "page", required = false, defaultValue = "1")
        ...
        if (pageNum != 1) {
            Link prevPageLink = linkTo(methodOn(TagController.class).read(pageNum - 1, pageSize))
                    .withRel("Previous page");
            collectionLinks.add(prevPageLink);
        }
        Link currentPageLink = linkTo(methodOn(TagController.class).read(pageNum, pageSize))
    ...
    public TagDto read(@PathVariable Long id) {
        ...
        Link self = linkTo(methodOn(TagController.class).read(id)).withSelfRel();
        dto.add(self);
        return dto;
    }
    ...
    public TagDto create(@RequestBody TagDto tag) {
        ...
        Link toCreatedLink = linkTo(methodOn(TagController.class).read(dto.getId())).withRel("Read created");
